...
const DEFAULT_DMT_TOOLS = [
  {
    ...
    type: "grille",
    rows: [
      { phase: "Accueil / identification", cible: "≤ 15s", vigilance: "Une seule formule d'accueil, pas de répétition.", commentaire: "Le conseiller maîtrise la prise de contact ; l'écart porte sur la rapidité d'entrée en matière, pas sur le relationnel." },
      { phase: "Découverte du besoin", cible: "≤ 45s", vigilance: "3 questions fermées max avant de traiter.", commentaire: "La compréhension du besoin est là ; l'écart vient d'un enchaînement de questions trop long avant de passer à l'action." },
      { phase: "Traitement / réponse", cible: "≤ 90s", vigilance: "Une seule recherche outil ; si 2e recherche, basculer en mode expert.", commentaire: "La réponse apportée est correcte ; l'écart porte sur l'efficacité de la recherche outil, pas sur la connaissance produit." },
      { phase: "Reformulation", cible: "≤ 20s", vigilance: "Une phrase, pas un résumé complet du dossier.", commentaire: "La reformulation existe mais est trop détaillée ; l'écart est sur la concision, pas sur la rigueur du suivi." },
      { phase: "Clôture", cible: "≤ 20s", vigilance: "Script standard (voir outil ci-dessous), pas d'improvisation.", commentaire: "La clôture est présente mais s'écarte du script standard ; l'écart porte sur la structure, pas sur le ton avec le client." },
    ],
  },
  {
    id: "script-wrapup",
...
const DEFAULT_COMMERCE_TOOLS = [
  {
    ...
    type: "grille",
    rows: [
      { phase: "Découverte du besoin", cible: "≤ 60s", vigilance: "Au moins 1 question de situation avant toute proposition.", commentaire: "Le conseiller connaît l'offre ; l'écart porte sur le fait de proposer trop tôt, avant d'avoir fait émerger le besoin." },
      { phase: "Question d'implication", cible: "Systématique", vigilance: "Le client doit formuler lui-même l'impact du problème, pas le conseiller.", commentaire: "L'échange reste courtois et fluide ; l'écart est que c'est le conseiller qui formule l'impact à la place du client." },
      { phase: "Proposition commerciale", cible: "Reliée au besoin exprimé", vigilance: "L'offre cite un mot ou une contrainte dite par le client, pas un pitch générique.", commentaire: "L'offre présentée est adaptée sur le fond ; l'écart est qu'elle n'est pas reliée explicitement aux mots du client." },
      { phase: "Traitement de l'objection", cible: "1 reformulation + 1 argument", vigilance: "Reformuler l'objection avant d'y répondre, jamais répondre à chaud.", commentaire: "L'argument apporté est pertinent ; l'écart porte sur l'absence de reformulation avant d'y répondre." },
      { phase: "Conclusion", cible: "Question fermée explicite", vigilance: "Une vraie question de conclusion posée, pas une offre laissée en suspens.", commentaire: "La proposition est claire ; l'écart est qu'elle n'est pas suivie d'une vraie question de conclusion." },
    ],
  },
  {
...
function exportPAAPdf(p, equipesList) {
  ...
  win.document.write(`
    ...
    <body>
      ...
      <div class="meta">${escapeHtml(themeLabel(p.theme))} · ${escapeHtml(equipeLabel(p.equipe, equipesList))} · Généré le ${escapeHtml(formatDateTime(p.createdAt))}</div>
      ...
      <div class="footer">Généré via Assistant Conseillers — ${escapeHtml(formatDateTime(new Date().toISOString()))}</div>
    </body>
    </html>
  `);
  win.document.close();
...
export default function AssistantConseillers() {
  ...
  // Pré-remplissage depuis un lien externe (ex. le portail d'écoute interne) :
  // ?conseiller=Jean+Dupont pré-remplit le nom du conseiller dans tous les formulaires
  // et dans la génération de PAA depuis la grille d'écoute. Sans ce paramètre, rien ne
  // change. Le paramètre est nettoyé de l'URL après lecture, pour rester discret si
  // l'onglet est partagé ou mis en favori ensuite.
  useEffect(() => {
    const params = new URLSearchParams(window.location.search);
    const conseiller = params.get("conseiller");
    if (conseiller && conseiller.trim()) {
      const nom = conseiller.trim();
      setDmtNom(nom);
      setComNom(nom);
      setDiscNom(nom);
      setFidNom(nom);
      setGrilleConseillerNom(nom);
      const url = new URL(window.location.href);
      url.searchParams.delete("conseiller");
  ...
  function genererPAADepuisConstats(tool, theme) {
    const kos = tool.rows
      .map((row, ri) => ({ phase: row.phase, vigilance: row.vigilance, commentaire: row.commentaire, statut: grilleNotes[tool.id]?.[ri] }))
    ...
    const commentairesBanque = kos
      .map((k) => k.commentaire)
      .filter(Boolean)
      .join(" ");
    const commentaireFinal = [commentairesBanque, grilleCommentaire.trim()].filter(Boolean).join(" ") || "—";
  ...
  async function exportExcelReport() {
    ...
    sheetSynth.getCell("A2").value = `Généré le ${formatDateTime(new Date().toISOString())}`;
    ...
    const headers = [
      "Conseiller", "Équipe", "Thème", "Cause", "Généré le",
      "Métrique", "Valeur initiale", "Cible", "Statut", "Objectif atteint le", "Délai (jours)",
    ];
    ...
    history.forEach((p) => {
      ...
      const row = sheetDetail.addRow([
        ...
        reached ? formatDateTime(p.atteintAt) : "",
        delaiJours,
      ]);
    ...
    const widths = [22, 20, 18, 26, 20, 20, 14, 12, 12, 22, 12];
                    ...
                    {tool.type === "grille" && (() => {
                      ...
                      return (
                        <div className="mt-3 rounded-md px-3 py-2 space-y-2" style={{ background: "#FDECEC" }}>
                          ...
                          <input
                            type="text"
                            placeholder="Ajouter un détail au commentaire automatique (optionnel)"
              ...
              >
                ...
                >
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-[10px]" style={{ color: "#9FADC2" }}>
                      · Généré le {formatDateTime(p.createdAt)}
                    </span>
                  </div>
                  <div className="flex items-center gap-2">
                    {p.status !== "atteint" && (
...
function rowsToText(tool) {
  if (!tool.rows) return "";
  if (tool.type === "grille") {
    return tool.rows.map((r) => `${r.phase} | ${r.cible} | ${r.vigilance} | ${r.commentaire || ""}`).join("\n");
  }
  if (tool.type === "fiche") {
...
function textToRows(type, text) {
  ...
  return lines.map((line) => {
    ...
    if (type === "grille") {
      return { phase: parts[0] || "", cible: parts[1] || "", vigilance: parts[2] || "", commentaire: parts[3] || "" };
    }
      ...
      <div className="bg-white rounded-xl w-full max-w-xl max-h-[85vh] overflow-y-auto p-6">
        ...
        <div className="space-y-3">
          ...
          {type === "script" ? (
            ...
            <Field label={`Lignes (une par ligne, format : ${type === "grille" ? "phase | cible | vigilance | commentaire factuel" : "motif | où chercher | cible"})`}>
              {type === "grille" && (
                <div className="text-[11px] mb-1" style={{ color: SKY }}>
                  Le commentaire factuel est repris automatiquement dans le PAA généré quand cette phase est cochée KO.
                </div>
              )}
