...
export default function AssistantConseillers() {
  ...
  function genererPAADepuisConstats(tool) {
    const notes = tool.rows
      .map((row, ri) => ({ phase: row.phase, note: grilleNotes[tool.id]?.[ri] }))
      .filter((r) => r.note && r.note.trim());

    if (notes.length === 0) {
      showToast("Note au moins un écart avant de générer un plan.");
      return;
    }
    ...
    const entry = {
      ...
      actuel: "",
      cible: "",
      indicateurEcart: `Écarts constatés en écoute (${tool.title}) : ${notes.map((n) => n.phase).join(", ")}.`,
      actionsFactuelles: notes.map((n) => `${n.phase} — ${n.note.trim()}`),
      commentaire: "Constat réalisé en écoute directe par le superviseur, phase par phase.",
      suiviSmart: `Spécifique : corriger les écarts constatés sur ${notes.map((n) => n.phase).join(", ")}. Mesurable : nouvelle écoute de contrôle. Atteignable : actions ciblées ci-dessus. Réaliste : basé sur une observation réelle, pas une estimation. Temporel : point de suivi au ${addWeeks(3)}.`,
      felicitation: "Écarts résorbés lors de la prochaine écoute de contrôle : à valoriser auprès du conseiller.",
    };

    addEntry(entry);
    setGrilleNotes((s) => ({ ...s, [tool.id]: {} }));
    setGrilleConseillerNom("");
    showToast("PAA généré à partir des constats d'écoute.");
  }
              ...
              <div key={tool.id || idx} className="rounded-lg border p-4 bg-white" style={{ borderColor: "#E2E8F5" }}>
                ...
                {(tool.type === "grille" || tool.type === "fiche") && (
                  <div className="overflow-x-auto">
                    {tool.type === "grille" && (
                      <div className="mb-3 text-[11px]" style={{ color: SKY }}>
                        Note un écart uniquement sur les phases concernées, avec l'action concrète à mener — le reste peut rester vide.
                      </div>
                    )}
                    <table className="w-full text-xs">
                        ...
                        <tr style={{ color: VIOLET }}>
                          {tool.type === "grille" ? (
                            <>
                              ...
                              <th className="text-left font-bold uppercase py-1">Écart constaté + action</th>
                            </>
                          ) : (
                            <>
                      ...
                      <tbody>
                        {tool.rows.map((row, ri) => {
                          const note = grilleNotes[tool.id]?.[ri] || "";
                          return (
                              ...
                              {tool.type === "grille" ? (
                                <>
                                  ...
                                  <td className="py-1.5 align-top">
                                    <input
                                      type="text"
                                      placeholder="ex. reformulation trop longue, à raccourcir"
                                      className="w-full rounded border px-2 py-1 text-xs"
                                      style={{ borderColor: note ? "#D9534F" : "#D8E0F0", background: note ? "#FDECEC" : "white" }}
                                      value={note}
                                      onChange={(e) =>
                                        setGrilleNotes((s) => ({
                                          ...s,
                                          [tool.id]: { ...(s[tool.id] || {}), [ri]: e.target.value },
                                        }))
                                      }
                                    />
                                  </td>
                      ...
                      return (
                        <div className="mt-3 rounded-md px-3 py-2 space-y-2" style={{ background: "#FDECEC" }}>
                          <div className="text-xs font-semibold" style={{ color: "#B3261E" }}>
                            {notes.length} écart{notes.length > 1 ? "s" : ""} noté{notes.length > 1 ? "s" : ""} — prêt à devenir un plan d'accompagnement.
                          </div>
                          <div className="flex items-center gap-2">
                            ...
                            />
                            <button
                              onClick={() => genererPAADepuisConstats(tool)}
