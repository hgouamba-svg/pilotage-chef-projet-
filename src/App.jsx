...
// equipeNames : config dynamique { A: "nom personnalisé", ... }, chargée depuis le
// serveur. Sans elle (ou pour un id absent), on retombe sur les noms par défaut.
function equipeLabel(id, equipeNames) {
  if (equipeNames && equipeNames[id]) return equipeNames[id];
  return DEFAULT_EQUIPES.find((e) => e.id === id)?.label || id || "—";
}
...
function exportPAAPdf(p, equipeNames) {
  ...
  win.document.write(`
    ...
    <body>
      ...
      <div class="meta">${escapeHtml(themeLabel(p.theme))} · ${escapeHtml(equipeLabel(p.equipe, equipeNames))} · ${escapeHtml(new Date(p.createdAt).toLocaleDateString("fr-FR"))}</div>
...
export default function AssistantConseillers() {
  ...
  const [equipeNames, setEquipeNames] = useState({});
  ...
  const [dmtTools, setDmtTools] = useState(DEFAULT_DMT_TOOLS);
  ...
  // Charge la bibliothèque d'outils DMT personnalisée (ajouts/modifs d'un admin) ;
  // sinon les 3 outils par défaut restent utilisés.
  useEffect(() => {
    if (!token) return;
    (async () => {
      try {
        ...
        if (!res.ok) return;
        const data = await res.json();
        if (data && Array.isArray(data)) setDmtTools(data);
      } catch (e) {
        // pas grave : on garde les valeurs par défaut
      }
    })();
  ...
  // Charge les noms d'équipe personnalisés (renommés par un superviseur ou un admin) ;
  // sinon les noms par défaut (Équipe A — FO, etc.) restent utilisés.
  useEffect(() => {
    if (!token) return;
    (async () => {
      try {
        ...
        if (!res.ok) return;
        const data = await res.json();
        if (data) setEquipeNames(data);
      } catch (e) {
        // pas grave : on garde les noms par défaut
      }
    })();
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [token]);
  ...
  async function handleRenameEquipe(id) {
    const current = equipeLabel(id, equipeNames);
    ...
    try {
      const res = await fetch("/api/equipes", {
        method: "POST",
        headers: authHeaders(),
        body: JSON.stringify({ id, label: next.trim() }),
      });
      const data = await res.json();
      if (!res.ok) {
  ...
  return (
    <div className="min-h-screen" style={{ background: PAPER, fontFamily: "Arial, sans-serif" }}>
        ...
        <div className="max-w-5xl mx-auto">
          ...
          <div className="flex flex-wrap items-center gap-3 mt-4">
            <span className="text-xs px-2.5 py-1 rounded-full" style={{ background: "#20406E", color: "white" }}>
              {user.displayName} · {user.role === "admin" ? "Administrateur" : equipeLabel(user.equipe, equipeNames)}
            </span>
      ...
      <div className="max-w-5xl mx-auto px-6 sm:px-10 py-8">
        ...
        {user.role === "admin" && (
          <div className="mb-5 flex items-center gap-3">
            ...
            >
              {DEFAULT_EQUIPES.map((eq) => (
                <option key={eq.id} value={eq.id}>
                  {equipeLabel(eq.id, equipeNames)}
                </option>
              ))}
            </select>
            <button
                    ...
                    <div className="rounded-lg border p-4 bg-white" style={{ borderColor: "#E2E8F5" }}>
                      ...
                      {Object.entries(parEquipe).map(([eq, count]) => (
                        <div key={eq} className="flex items-center justify-between text-sm py-1">
                          <span style={{ color: NAVY }}>{equipeLabel(eq, equipeNames)}</span>
                          <span className="font-semibold" style={{ color: SKY }}>{count}</span>
                        </div>
                      ))}
                    </div>
                ...
                >
                  <div className="flex items-center gap-2">
                    ...
                    {p.equipe && (
                      ...
                      >
                        {equipeLabel(p.equipe, equipeNames)}
                      </span>
                    )}
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <button
                      onClick={() => exportPAAPdf(p, equipeNames)}
                      className="text-xs flex items-center gap-1 px-2.5 py-1 rounded-full transition hover:opacity-80"
                      style={{ background: "#EEF1FA", color: SKY }}
                    >
      ...
      {showUsers && user.role === "admin" && (
        <UserManagementModal token={token} equipeNames={equipeNames} onClose={() => setShowUsers(false)} showToast={showToast} />
      )}
...
function UserManagementModal({ token, equipeNames, onClose, showToast }) {
          ...
          {newRole !== "admin" && (
            <Field label="Équipe">
              <select className={selectCls} style={{ borderColor: "#D8E0F0" }} value={newEquipe} onChange={(e) => setNewEquipe(e.target.value)}>
                {DEFAULT_EQUIPES.map((eq) => (
                  <option key={eq.id} value={eq.id}>{equipeLabel(eq.id, equipeNames)}</option>
                ))}
              </select>
            </Field>
          )}
              ...
              <div key={u.id} className="flex items-center justify-between text-sm rounded-md px-3 py-2" style={{ background: "#F7F9FD" }}>
                <div>
                  ...
                  <span style={{ color: SKY }}>
                    · {u.username} · {u.role === "admin" ? "Administrateur" : equipeLabel(u.equipe, equipeNames)}
                  </span>
                </div>
...
function ManualEntryModal({ user, defaultEquipe, equipeNames, onClose, onCreate }) {
...
function ToolsEditorModal({ token, dmtTools, setDmtTools, onClose, showToast }) {
  const [selectedId, setSelectedId] = useState(dmtTools[0]?.id || null);
  ...
  useEffect(() => {
    ...
    const t = dmtTools.find((x) => x.id === selectedId);
    if (t) {
      setTitle(t.title || "");
  ...
  }, [selectedId, creatingNew, dmtTools]);

  async function persistTools(next) {
    setSaving(true);
    try {
  ...
  async function handleSave() {
    ...
    let next;
    if (creatingNew) {
      next = [...dmtTools, toolData];
    } else {
      next = dmtTools.map((t) => (t.id === selectedId ? toolData : t));
    }
  ...
  async function handleDelete() {
    if (dmtTools.length <= 1) {
      showToast("Il doit rester au moins un outil.");
      return;
    }
    const next = dmtTools.filter((t) => t.id !== selectedId);
    const ok = await persistTools(next);
    if (ok) {
